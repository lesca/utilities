...
# nctar - receiver side
nctar() { [ "$1" = "" ] && port=9999 || port=$1 ; nc -l $port | tar --checkpoint=100000 --checkpoint-action=echo="#%u: %T" -xf - ; }

# ncssl name [dir]
ncssl() { [ "$1" = "" ] && name="base" ; [ "$2" = "" ] && des=encrypted || des=$2 ; mkdir -p $des ; nc -l 9999 | tar --checkpoint=100000 --checkpoint-action=echo="#%u: %T" -xf - | openssl enc -aes-256-cbc -pass file:passfile | split -a3 -d -b 2G - $des/$name.part ; }
