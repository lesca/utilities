...
# ncssl name [dir]
ncssl() { [ "$1" = "" ] && name="base" ; [ "$2" = "" ] && des=encrypted || des=$2 ; mkdir -p $des ; nc -dN -l 9999 | tar --checkpoint=100000 --checkpoint-action=echo="#%u: %T" -xf - | openssl enc -aes-256-cbc -pass file:$HOME/.passfile | split -a3 -d -b 2G - $des/$name.part ; }
